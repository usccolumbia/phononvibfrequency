...
class METRICS:
    ...
    def custom__err_bynumNOdes(self,tensor1, tensor2, Num,error_func):
        error1          = torch.tensor(0.0).to(self.dv) 
        Num             = Num.cpu().detach().numpy().astype(int)
        #3print(Num)
        #3exit()
        #3Num = 
        #print(tensor1.shape)
        ...
        # split the tensors here
        for i in range(len(tensor1)):

            #print(tensor1[i].shape)
            #print(tensor2[i].shape)

            crystal1 = tensor1[i][:Num[i]]
            crystal2 = tensor2[i][:Num[i]]

            temp_mse = error_func(crystal1,crystal2)

            error1  += temp_mse

        avg_err      = error1/len(tensor1)
        return avg_err

    def __call__(self,which_phase,tensor_pred,tensor_true,NUM,measure=1):
        if measure == 1:
            if   which_phase == 'training'  : 
                loss         = self.custom__err_bynumNOdes(tensor_pred,tensor_true, NUM,self.criterion)#3.criterion(tensor_pred,tensor_true)
                self.training_measure1 += loss
            elif which_phase == 'validation':
                loss         = self.custom__err_bynumNOdes(tensor_pred,tensor_true, NUM,self.criterion)
                self.valid_measure1    += loss
        else:
            if   which_phase == 'training'  :
                loss         = self.custom__err_bynumNOdes(tensor_pred,tensor_true, NUM,self.eval_func)#(tensor_pred,tensor_true) 
                self.training_measure2 += loss
            elif which_phase == 'validation':
                loss         = self.custom__err_bynumNOdes(tensor_pred,tensor_true, NUM,self.eval_func)#self.eval_func(tensor_pred,tensor_true)
...
class POSCAR_Dataset(Dataset):
    def __init__(self,part_data=None,norm_obj=None,normalization=None,max_num_nbr=12, radius=8, dmin=0, step=0.2,cls_num=3,root_dir = 'DATA/POSCAR-data/'):
        self.root_dir      = root_dir
        self.max_num_nbr, self.radius = max_num_nbr, radius
        self.normalizer    = norm_obj
        self.normalization = normalization
        self.full_data     = part_data
        self.ari           = AtomCustomJSONInitializer('DATA/ATOM_INIT/atom_init.json')
    ...
    def __getitem__(self, idx):
        temp_arr =self.full_data.iloc[idx].values.reshape(-1)
        poscar_id = temp_arr[0]
        target = temp_arr[3:]
        crystal = Structure.from_file(os.path.join(self.root_dir, poscar_id+'/POSCAR')) # This function can also take in CIF files
        ...
        target       = torch.Tensor(target.astype(float)).float().view(1,-1)

        coordinates = torch.tensor(g_coords) 
        enc_compo   = self.encoder_elem.encode(crystal.composition)
        return (atom_fea, nbr_fea, nbr_fea_idx),groups,enc_compo,coordinates, target, poscar_id,[crystal[i].specie for i in range(len(crystal))]
